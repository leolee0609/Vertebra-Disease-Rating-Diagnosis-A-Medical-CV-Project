function [a2, b2, training_set] = fit_straight_line_at_right_corner_along_bottom_outline(ROI_msk, perc, ifshow)
    % this function returns the coefficients of the tangent line at the
    % right-bottom point along the bottom outline, which is obtained using
    % linear regression. We first get
    % the closest perc% points to the right-bottom point on the bottom
    ...
    % find the top col_count closest points on the bottom outline
    bottom_outline_size = size(bottom_outline);
    bottom_outline_size = bottom_outline_size(1);


    % how many points are we considering for training the linear regr?
    col_count = floor(perc * bottom_outline_size);
    ...
    [a2, b2] = novel_linear_regression_trainer(feature, label);
