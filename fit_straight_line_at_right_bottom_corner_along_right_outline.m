function [a4, b4, training_set] = fit_straight_line_at_right_bottom_corner_along_right_outline(ROI_msk, perc, ifshow)
    % this function returns the coefficients of the tangent line at the
    % right-bottom point along the right outline, which is obtained using
    % linear regression. We first get
    % the closest perc% points to the right-bottom point on the right
    ...
    % find the top col_count closest points on the right outline
    right_outline_size = size(right_outline);
    right_outline_size = right_outline_size(1);


    % how many points are we considering for training the linear regr?
    col_count = floor(perc * right_outline_size);
    remaining_set = right_outline;
    ...
    for k = 1: col_count
        ...
        for t = 1: remaining_count
            dis = sqrt((remaining_set(t, 1) - rbp(1))^2 + (remaining_set(t, 2) - rbp(2))^2);
    ...
    [a4, b4] = novel_linear_regression_trainer_for_right_outline(feature, label);
