function [mean_greyscale, mean_greyscale_whole, ratio, std_diff] = left_bottom_mean_greyscale_calculator(img, ROI_msk, perc, ifshow)
    % this function calculates the mean greyscale-value around the
    % left-bottom corner. Radius is defined by perc. 
    % Note that img must be greyscaled
    ...
    for i = 1: row
        for j = 1: col
            ...
            if bina_mask(i, j) ~= 0 
                ...
                if (i - lbp(1))^2 + (j - lbp(2))^2 < radius^2
